...
import {
  ...
  resetKpiScopeConfig,
  saveKpiScopeConfig,
} from "@/features/kpi-scope/api";
import {
  KPI_SCOPE_GROUP_LABEL,
  ROLE_LEVEL_HINT,
  sameConfig,
  type KpiScope,
  type KpiScopeConfigItem,
  type KpiScopeGroup,
  type KpiScopeMeta,
} from "@/features/kpi-scope/types";
import { getApiErrorMessage } from "@/lib/api-client";
import { cn } from "@/lib/utils";

type ViewMode = "MATRIX" | "ROLE";

export function KpiScopeConfigView() {
  const { data, isLoading, mutate } = useSWR(
    kpiScopeKeys.config,
    fetchKpiScopeConfig,
  );

  const [items, setItems] = useState<KpiScopeConfigItem[]>([]);
  ...
  const groups = useMemo(() => {
    const map = new Map<KpiScopeGroup, KpiScopeMeta[]>();
  ...
  const patch = (roleCode: string, next: Partial<KpiScopeConfigItem>) => {
    setItems((prev) =>
      prev.map((item) =>
  ...
  const toggleScope = (roleCode: string, scope: KpiScope) => {
    setItems((prev) =>
      prev.map((item) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success("Đã lưu cấu hình phạm vi giao KPI.");
      await mutate();
    } catch (error) {
  ...
  const reset = async () => {
    setSaving(true);
    try {
      const fresh = await resetKpiScopeConfig();
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          <h1 className="font-display text-2xl font-semibold tracking-tight">
            Phân quyền giao KPI theo phạm vi
          </h1>
        ...
        <Card>
          <CardContent className="space-y-3 pt-6">
                  ...
                  {scopeMeta.map((meta) => (
                    <TableRow key={meta.key}>
                      ...
                      {items.map((item) => {
                        ...
                        return (
                          <TableCell key={item.roleCode} className="text-center">
                            <button
                              type="button"
                              disabled={!item.isEnabled}
                              onClick={() => toggleScope(item.roleCode, meta.key)}
                              className={cn(
            ...
            <p className="text-xs text-muted-foreground">
              ...
              được KPI cho bất kỳ phạm vi nào.
            </p>
          </CardContent>
        </Card>
      ) : (
