...
} from "@/features/mission-summary-report/components/manual-draft-list";
import { SummaryCandidatePicker } from "@/features/mission-summary-report/components/summary-candidate-picker";
import { periodLabel } from "@/features/mission-summary-report/types";
import { fetchPersonalMissionRecipients } from "@/features/personal-mission/api";
 ...
 * đủ bộ - việc của chính đơn vị, việc phối hợp, việc đột xuất không đi qua nhiệm vụ
...
export function CreateReportWizard({
  ...
  const { data: departments = [] } = useSWR(
    ["departments", "all", "mission-summary"],
    fetchDepartments,
  );
  const { data: recipients } = useSWR(
    open ? ["personal-mission", "recipients", "mission-summary"] : null,
