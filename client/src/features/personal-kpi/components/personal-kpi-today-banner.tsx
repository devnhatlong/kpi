...
import type { PersonalKpiDailyReport } from "@/features/personal-kpi/api";
  ...
  switch (status) {
    ...
    case "sent":
      return {
        ...
        className: kpiTone.warning.soft,
    ...
    case "completed":
      return {
        ...
        className: kpiTone.success.soft,
        action: "Xem báo cáo hôm nay" as const,
        actionIcon: "eye" as const,
      };
    default:
...
type PersonalKpiTodayBannerProps = {
  todayYmd: string;
  todayReport?: PersonalKpiDailyReport | null;
  loading?: boolean;
  onCreateToday: () => void;
  onOpenToday: () => void;
};

export function PersonalKpiTodayBanner({
  todayYmd,
  todayReport,
  loading = false,
  onCreateToday,
  onOpenToday,
}: PersonalKpiTodayBannerProps) {
  const { user } = useAuth();
