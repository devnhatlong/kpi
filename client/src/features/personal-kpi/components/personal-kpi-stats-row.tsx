...
import type { PersonalKpiDashboardSummary } from "@/features/personal-kpi/api";
import { kpiTone } from "@/features/personal-kpi/status-styles";
import { cn } from "@/lib/utils";

type PersonalKpiStatsRowProps = {
  summary?: PersonalKpiDashboardSummary | null;
  loading?: boolean;
  onOpenToday?: () => void;
};
...
export function PersonalKpiStatsRow({
  summary,
  loading = false,
  onOpenToday,
}: PersonalKpiStatsRowProps) {
        ...
        <div className="flex items-center gap-2.5">
          <span
            className={cn(
              ...
              streakDays > 0 ? kpiTone.success.icon : kpiTone.neutral.icon,
            )}
          >
            {streakDays > 0 ? (
      ...
      >
        <div className="flex items-center gap-2.5">
          <span
            className={cn(
              "flex size-9 items-center justify-center rounded-full",
              rejectedCount > 0 ? kpiTone.danger.icon : kpiTone.warning.icon,
            )}
          >
