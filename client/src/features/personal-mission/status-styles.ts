import type { PersonalMissionStatus } from "@/features/personal-mission/types";

/**
 * Palette nhiệm vụ cá nhân - nền nhạt + chữ đậm, học theo UI báo cáo:
 ...
 */
export const missionStatusPillClass: Record<PersonalMissionStatus, string> = {
  DRAFT:
...
export function personalMissionStatusBadgeClass(status: PersonalMissionStatus) {
  // Bản ghi cũ / trạng thái lạ vẫn phải ra pill đọc được, không để rơi undefined.
  return missionStatusPillClass[status] ?? missionStatusPillClass.DRAFT;
}

export const missionTone = {
  success: {
