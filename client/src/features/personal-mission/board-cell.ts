import type { PersonalMissionBoardRow } from "@/features/personal-mission/api";
import {
  effectiveMaxScore,
  footerMode,
  formulaValueSource,
...
export function rowDepartmentRef(row: PersonalMissionBoardRow): {
  id: string;
  code: string;
  name: string;
} {
...
export function rowDepartmentKey(row: PersonalMissionBoardRow): string {
  return rowDepartmentRef(row).id;
}

/** Người gửi / cán bộ đứng tên dòng, kèm đơn vị - cột hay dùng ở mọi bảng tổng. */
export function rowSenderLabel(row: PersonalMissionBoardRow) {
  return {
