...
type PanelRow = CriteriaRow &
  Pick<PersonalCriterionRow, "reviewValues" | "reviewCatalogValues">;
...
const LOG_TITLE: Record<PersonalMissionProgressLog["type"], string> = {
 ...
 */
function CriteriaLog({ logs }: { logs: PersonalMissionProgressLog[] }) {
...
function CriteriaStatusNote({
  ...
  status: keyof typeof PERSONAL_MISSION_STATUS_LABEL;
  period: string;
  returnReason: string;
  reviewNote: string;
  ...
  if (status === "PENDING" || status === "APPROVED") {
    return (
      <p className="rounded-lg border bg-muted/40 px-3 py-2 text-xs">
        ...
        Bảng đã gửi lên trên ({PERSONAL_MISSION_STATUS_LABEL[status]}). Vẫn sửa
        được cho tới khi chỉ huy chốt, nhưng mỗi lần sửa đều để lại vết.
      </p>
    );
  }
