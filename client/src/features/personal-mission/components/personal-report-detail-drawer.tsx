...
import { AxisTaskTable } from "@/features/personal-mission/components/axis-task-table";
import {
  CriteriaTable,
  type CriteriaRow,
...
import { PersonalTaskForm } from "@/features/personal-mission/components/personal-task-form";
...
import { personalMissionStatusBadgeClass } from "@/features/personal-mission/status-styles";
...
type AxisGroup = {
  ...
  contents: Array<{
    ...
    items: PersonalMissionItem[];
  }>;
};

function groupByAxisContent(items: PersonalMissionItem[]): AxisGroup[] {
...
export function PersonalReportDetailDrawer({
  ...
  const [items, setItems] = useState<PersonalMissionItem[]>([]);
  ...
  const [reasonItem, setReasonItem] = useState<PersonalMissionItem | null>(
    null,
  );
  ...
  */
  ...
  const canSendCriteria = criteriaFilled && (criteriaSheet?.canEdit ?? false);
  ...
  */
  ...
  const criteriaRows: CriteriaRow[] = (criteriaSheet?.rows ?? []).map(
    (row) => ({
  ...
  const handleSend = async (payload: SubmitPersonalMissionPayload) => {
    ...
    setSending(true);
    try {
      const result = await submitPersonalMissionReport(reportDate, {
        ...payload,
            ...
            <div className="flex flex-wrap gap-1 pt-1">
              {Object.entries(
                ...
                <Badge
                  key={status}
                  variant="secondary"
                  className={personalMissionStatusBadgeClass(
                    status as PersonalMissionStatus,
                  )}
                >
                  {PERSONAL_MISSION_STATUS_LABEL[
                    status as PersonalMissionStatus
          ...
          {criteriaSheet?.sheetId ? (
            <section className="space-y-2 rounded-lg border bg-card p-3">
              <header className="flex flex-wrap items-center gap-2">
                ...
                >
                  {PERSONAL_MISSION_STATUS_LABEL[criteriaSheet.reviewStatus]}
                </Badge>
                      ...
                      >
                        {(columns) =>
                          content.items.map((item, taskIndex) => {
                            ...
                            const canEdit = canEditPersonalMission(item.status);
                            return (
                              <PersonalTaskForm
                                key={item.id}
                                index={stt}
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            {reasonItem && canEditPersonalMission(reasonItem.status) ? (
              <Button
                onClick={() => {
