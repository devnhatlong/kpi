...
import { scorePersonalMission } from "@/features/personal-mission/api";
...
import { ResultScoreForm } from "@/features/personal-mission/components/result-score-form";
import {
  resultColumns,
  scoreColumns,
  type ScoreColumns,
...
import type { PersonalMissionItem } from "@/features/personal-mission/types";
...
function ScoreForm({
  ...
  /**
   * Chốt sớm: tiến độ chưa tới 100%, hoặc cán bộ chưa nhập số nào.
   *
  ...
  const submit = async () => {
    setSaving(true);
    try {
      await scorePersonalMission(item.id, { values, note });
      await onScored();
          ...
          >
            ...
            <div className="min-w-0 space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                nhật tiến độ được nữa và điểm nhiệm vụ tính theo số chấm bên
                dưới. Muốn cán bộ làm tiếp thì đóng form này và bấm{" "}
...
type ReviewScoreDialogProps = {
  ...
  /** tiến độ hiện tại, hiện ở dòng tóm tắt. */
  progressPercent: number | null;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>Chỉ huy chấm điểm nhiệm vụ</DialogTitle>
          <DialogDescription>
          ...
          <>
            <div className="flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/5 p-3 text-sm">
              ...
              <p className="text-muted-foreground">
                Mẫu nhiệm vụ của trục này chưa cấu hình công thức điểm nên chưa
                chấm được. Vào Cấu hình form nhiệm vụ › Mẫu bảng nhiệm vụ để
                khai cột mẫu số và các cột tử số.
              </p>
            </div>
            <DialogFooter>
              <Button
