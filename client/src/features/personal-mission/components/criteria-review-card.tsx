...
type CriteriaReviewCardProps = {
  blocks: PersonalMissionBoardCriteriaBlock[] | null;
...
export function CriteriaReviewCard({
   ...
   */
  const doForward = async (payload: SubmitPersonalMissionPayload) => {
    if (!forwardSheet) return;
    setBusyId(forwardSheet._id);
    try {
      const result = await forwardPersonalMission({
