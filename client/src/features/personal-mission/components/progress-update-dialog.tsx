...
} from "@/features/personal-mission/review-scores";
...
type MilestoneSliderProps = {
  label: string;
  /** Tên cột trong mẫu nhiệm vụ - để đối chiếu ngược lại bảng nhập. */
  columnTitle: string;
        ...
        <div className="min-w-0">
          ...
          {columnTitle && columnTitle !== label ? (
            <p className="truncate text-xs text-muted-foreground">
              Cột &quot;{columnTitle}&quot; trong mẫu nhiệm vụ
            </p>
          ) : null}
        </div>
        <span className="shrink-0 text-sm tabular-nums">
          {scored === null ? (
            <>
              ...
              <span className={cn("font-semibold", missionTone.info.text)}>
                {percentNow}%
              </span>
            </>
          ) : scored !== percentSelf ? (
            <>
              ...
              <span
                className={cn(
                  "font-semibold",
                  lowered ? missionTone.danger.text : missionTone.success.text,
                )}
              >
                Chỉ huy {scored}%
              </span>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                position.className,
                mark.percent === percentNow
                  ? cn("font-semibold", missionTone.info.text)
...
function logTitle(log: PersonalMissionProgressLog): string {
  switch (log.type) {
    case "SUBMIT":
...
function logYmd(log: PersonalMissionProgressLog): string {
  return log.onDate || serverYmd(log.at);
}

/** Màu chấm của một mốc: đỏ là bị trả lại, xanh lá là chốt, còn lại là tiến độ. */
function logDotClass(type: PersonalMissionProgressLog["type"]): string {
...
function logTitleClass(type: PersonalMissionProgressLog["type"]): string {
  if (type === "RETURN") return missionTone.danger.text;
  if (type === "COMPLETE") return missionTone.success.text;
  if (type === "EDIT") return missionTone.warning.text;
  if (type === "SUBMIT") return missionTone.info.text;
  return "";
}
 ...
 */
function noteLabel(log: PersonalMissionProgressLog): string {
  switch (log.type) {
    case "SUBMIT":
...
function TaskTimeline({
  ...
  logs: PersonalMissionProgressLog[];
...
function isRollback(log: PersonalMissionProgressLog): boolean {
...
function ChangeLine({
  ...
  change: PersonalMissionProgressChange;
  columns: TrackingColumns;
}) {
  const label = {
    progress: "Tiến độ",
  ...
  return (
    <p
      className={cn(
        ...
        ok ? missionTone.success.text : "text-muted-foreground",
      )}
    >
      {ok ? (
...
type ProgressFormProps = {
  ...
  onRequestConfirm?: (item: PersonalMissionItem) => void;
  /** Chỉ huy chốt hoàn thành ngay trong màn chi tiết. */
  onComplete?: (item: PersonalMissionItem) => void;
  /** Chỉ huy sửa nội dung nhiệm vụ - mở form sửa của dòng đang xem. */
  onEdit?: (item: PersonalMissionItem) => void;
  /** Chỉ huy trả lại nhiệm vụ cho cấp dưới. */
  onReturn?: (item: PersonalMissionItem) => void;
};
...
function ProgressForm({
  ...
  */
  ...
  const sendable = canSendPersonalMission(item.status);
  const isReturned = item.status === "RETURNED";

  const save = async () => {
    setSaving(true);
    try {
      await updatePersonalMissionProgress(item.id, {
      ...
      <>
        <div className="flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-500/5 p-3 text-sm">
          ...
          <p className="text-muted-foreground">
            Mẫu nhiệm vụ của trục này chưa có cột tiến độ (ô phần trăm thuộc
            nhóm &quot;tiến độ&quot;) lẫn cột điểm trong công thức, nên chưa cập
            nhật được. Sửa mẫu tại Cấu hình form nhiệm vụ › Mẫu bảng nhiệm vụ,
            hoặc dùng &quot;Sửa chi tiết&quot; để nhập tay.
          </p>
        </div>
        <DialogFooter>
    ...
    <>
        ...
        <div className="space-y-4">
          ...
          */}
          ...
          {tracksProgress && columns.progressColumn ? (
            <MilestoneSlider
              label="Tiến độ nhiệm vụ (nhiệm vụ)"
          ...
          {columns.qualityColumn ? (
            <MilestoneSlider
              label="Chất lượng nhiệm vụ (nhiệm vụ)"
      ...
      <DialogFooter className="sm:items-center sm:justify-between">
        ...
        */}
        ...
        {onComplete && canCompletePersonalMission(item.status) ? (
