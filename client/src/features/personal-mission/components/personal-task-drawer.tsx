...
import { PersonalCriteriaPanel } from "@/features/personal-mission/components/personal-criteria-panel";
import type { Axis, WorkContent } from "@/features/mission-form-config/types";
import {
  entityId,
  footerMode,
  footerScoringLabel,
...
import { TaskEntryCard } from "@/features/personal-mission/components/task-entry-card";
import {
 ...
 *
 * Các ô trong thẻ dựng theo mẫu nhiệm vụ gán cho trục của nội dung đó - drawer không
 * tự đặt ra trường nào, đổi mẫu ở Cấu hình form nhiệm vụ là màn này đổi theo.
 */
export function PersonalTaskDrawer({
  ...
  */
  ...
  useEffect(() => {
    if (!focusKey) return;
    document
      .getElementById(`mission-entry-${focusKey}`)
                ...
                <div className="min-h-0 flex-1 space-y-4 overflow-auto px-3 py-3">
                  ...
                  {loading ? (
                    ...
                    libraryGroups.map((group) => {
                      ...
                      const open =
                        !!search.trim() || !collapsedAxes.has(group.axisId);
              ...
              {entries.length === 0 && !showCriteria ? (
                ...
                */}
                  {showCriteria ? (
                    <div
                      className={
                        activeTab === CRITERIA_TAB ? undefined : "hidden"
                      }
                    >
