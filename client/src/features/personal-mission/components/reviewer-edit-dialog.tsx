...
import { fetchWorkContentsAll } from "@/features/mission-form-config/api";
...
import { reviewerEditPersonalMission } from "@/features/personal-mission/api";
...
export function ReviewerEditDialog({
  ...
  */
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      await reviewerEditPersonalMission(item.id, {
        axisId,
        workContentId,
        ...
        <div className="max-h-[68vh] space-y-4 overflow-y-auto px-1">
          ...
          {!template ? (
            ...
            >
              ...
              Trục này chưa gán mẫu bảng nhiệm vụ nên không có ô nào để sửa. Vào
              Cấu hình form nhiệm vụ › Mẫu bảng nhiệm vụ để gán mẫu.
            </div>
          ) : task ? (
