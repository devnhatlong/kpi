...
import { ReportTemplateFormDialog } from "@/features/mission-form-config/components/report-builder/report-template-form-dialog";
import { scopeSummary } from "@/features/mission-form-config/components/report-builder/report-scope";
import {
  entityId,
...
function builderHref(template: ReportTemplate) {
  return `/mission/form-config/builder/${entityId(template)}`;
}
...
function totalScore(template: ReportTemplate, criteriaMaxScore: number) {
  const axisScore = (template.axisIds ?? []).reduce(
    (sum, axis) =>
      ...
      (typeof axis === "string"
        ? 0
        : ((axis as { maxScore?: number }).maxScore ?? 0)),
    0,
  );
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  items.map((item, index) => {
                    ...
                    const blockCount =
                      (item.axisIds?.length ?? 0) +
                      (item.includeCriteria ? 1 : 0);
                      ...
                      <TableRow key={id}>
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={
                              applied ? activeBadgeClass : inactiveBadgeClass
                            }
                          >
