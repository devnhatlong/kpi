...
import { AxisFormDialog } from "@/features/mission-form-config/components/axis-form-dialog";
import { CriteriaCatalogDialog } from "@/features/mission-form-config/components/report-builder/criteria-catalog-dialog";
...
import { LibraryRail } from "@/features/mission-form-config/components/report-builder/library-rail";
import { ReportPreviewDialog } from "@/features/mission-form-config/components/report-builder/report-preview-dialog";
import {
  scopeFingerprint,
  scopeFromTemplate,
  scopeIsComplete,
...
import { ScoringRulesDialog } from "@/features/mission-form-config/components/report-builder/scoring-rules-dialog";
import { TemplateComposer } from "@/features/mission-form-config/components/report-builder/template-composer";
import {
...
export function ReportBuilderView({ templateId }: { templateId: string }) {
  ...
  */
  if (target && !templatesSwr.isLoading && targetKey(target) !== loadedTarget) {
    ...
    const axisName =
      target.kind === "axis"
        ? (axes.find((axis) => entityId(axis) === target.axisId)?.name ??
          "trục")
        : "";
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[290px_minmax(0,1fr)]">
        ...
        <TemplateComposer
          ...
          criteriaMaxScore={criteriaMaxScore}
          includeCriteria={report.includeCriteria}
          onToggleCriteria={(includeCriteria) =>
            patchReport({ includeCriteria })
          }
          pickedAxisIds={orderedPicked}
      ...
      <Dialog open={scopeOpen} onOpenChange={setScopeOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Đơn vị nào dùng mẫu này khi nhập nhiệm vụ năm {year}. Thay đổi có
              hiệu lực sau khi bấm &quot;Lưu &amp; áp dụng mẫu&quot;.
            </DialogDescription>
          </DialogHeader>
          <ScopePicker
            value={report.scope}
