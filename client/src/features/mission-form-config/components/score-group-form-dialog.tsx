...
import {
  ...
  isScoreInGroupRange,
} from "@/features/mission-form-config/score-group.constants";
...
export function ScoreGroupFormDialog({
  ...
  const submit = async () => {
    ...
    if (
      ...
      toast.error(
        `Mức điểm từ phải nằm trong ${SCORE_GROUP_SCALE_MIN}..${SCORE_GROUP_SCALE_MAX}.`,
      );
      return;
    }
    ...
    if (
      ...
      toast.error(
        `Mức điểm đến phải nằm trong ${SCORE_GROUP_SCALE_MIN}..${SCORE_GROUP_SCALE_MAX}.`,
      );
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>{edit ? "Sửa nhóm điểm" : "Thêm nhóm điểm"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid gap-5 py-2">
          ...
          <div className="flex h-9 items-center justify-between rounded-lg border px-3">
            <Label htmlFor="score-group-max-inclusive">
              Bao gồm mức điểm đến
            </Label>
            <Switch
