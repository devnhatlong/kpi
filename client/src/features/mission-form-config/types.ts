...
export type WorkContent = {
  ...
  code: string;
  name: string;
  /** Cột "Nhiệm vụ" của bảng nhiệm vụ - admin khai sẵn, cán bộ chỉ đọc. */
  description?: string;
  /** Cột "Ghi chú" của bảng nhiệm vụ - admin khai sẵn (trần điểm của mục…). */
  note?: string;
...
export type WorkTask = {
  ...
  id?: string;
  code: string;
  /** Nguyên văn nhiệm vụ trong bảng nhiệm vụ. */
  name: string;
...
export type ColumnCatalog =
  "work_content" | "work_task" | "score_group" | "quality_level" | "criterion";
...
export function createDefaultTemplateDraft(): {
  ...
  return {
    ...
    headerGroups: [
      { id: progressGroupId, name: "Kết quả tiến độ (B)", children: [] },
      { id: qualityGroupId, name: "Kết quả chất lượng (C)", children: [] },
    ],
    columns: [
      column("stt", "STT", 60),
      ...
      freeColumn("progress_percent", "tiến độ %", 100, "number", [
        progressGroupId,
      ]),
      freeColumn("progress_self_score", "Điểm tự chấm", 110, "number", [
        progressGroupId,
      ]),
      freeColumn("quality_percent", "chất lượng %", 100, "number", [
        qualityGroupId,
      ]),
...
export function columnFromPreset(preset: FieldPreset): FormTemplateColumn {
  return {
    ...
    key:
      preset.key ??
      (preset.semanticKey === "custom" ? localId("field") : preset.semanticKey),
    title: preset.title,
    headerPath: [],
    width: preset.width,
    visible: true,
