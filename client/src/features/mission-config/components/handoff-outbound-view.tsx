...
export function HandoffOutboundView() {
  ...
  const { data: handoffs = [], mutate } = useSWR(
    workingDepartmentId
      ? [...missionConfigKeys.handoffs, "out", workingDepartmentId]
      : null,
    () =>
      fetchHandoffs({
  ...
  const { data: peers = [] } = useSWR(
    workingDepartmentId ? ["mission-peers", workingDepartmentId] : null,
              ...
              {!workingDepartmentId ? (
                ...
                handoffs.map((item) => (
                  <TableRow key={entityId(item)}>
                    ...
                    <TableCell>
                      {relationName(item.targetDepartmentId)}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Label>Kỳ nhiệm vụ (tuỳ chọn)</Label>
              <Select
