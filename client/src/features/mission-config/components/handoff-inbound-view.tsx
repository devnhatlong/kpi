...
export function HandoffInboundView() {
  ...
  const { data: handoffs = [], mutate } = useSWR(
    workingDepartmentId
      ? [...missionConfigKeys.handoffs, "in", workingDepartmentId]
      : null,
    () =>
      fetchHandoffs({
  ...
  const { data: sheets = [] } = useSWR(
    workingDepartmentId
      ? [...missionConfigKeys.sheets, "inbound", workingDepartmentId]
      : null,
  ...
  async function handleAccept() {
    ...
    try {
      ...
      await mutate();
      setAcceptOpen(false);
      toast.success("Đã đưa nhiệm vụ vào Form nhiệm vụ.");
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Form 3 - nhiệm vụ đơn vị ngang cấp giao sang; pick vào Form nhiệm
            vụ.
          </p>
        </div>
        <WorkingUnitSelect
              ...
              {!workingDepartmentId ? (
                ...
                handoffs.map((item) => (
                  <TableRow key={entityId(item)}>
                    ...
                    <TableCell>
                      {relationName(item.sourceDepartmentId)}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={acceptOpen} onOpenChange={setAcceptOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Đưa vào Form nhiệm vụ</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">{current?.title}</p>
            {sheets.length > 0 ? (
              <div className="space-y-2">
                <Label>Form nhiệm vụ hiện có</Label>
                <Select
                  value={sheetId || "__auto__"}
                  onValueChange={(v) => setSheetId(v === "__auto__" ? "" : v)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
