...
const SELECTED_TEMPLATE_KEY = "mission-selected-template-id";
...
export function MissionConfigView() {
  const { user } = useAuth();
  const canManageCatalog = userHasAnyRole(user, ["SUPER_ADMIN", "UNIT_ADMIN"]);
  ...
  const canAccessTemplateTab = canManageTemplate || canManageDepartmentTemplate;
  ...
  >("columns");
  const catalogGroupsQuery = useSWR(missionConfigKeys.groups(), () =>
    fetchWorkGroups(),
  );
  const catalogContentsQuery = useSWR(missionConfigKeys.contents(), () =>
    fetchWorkContents(),
  );
  const systemGroupsQuery = useSWR(missionConfigKeys.groups("SYSTEM"), () =>
    fetchWorkGroups("SYSTEM"),
  );
  const systemTemplatesQuery = useSWR(
    missionConfigKeys.templates("SYSTEM"),
    () => fetchMissionTemplates("SYSTEM"),
  );
  const systemContentsQuery = useSWR(missionConfigKeys.contents("SYSTEM"), () =>
    fetchWorkContents("SYSTEM"),
  );
  const tasksQuery = useSWR(missionConfigKeys.tasks, fetchTaskAssignments);
  ...
  const openCreate = () => {
    ...
    if (tab === "groups") {
      ...
      if (!selectedTemplate) {
        toast.error("Hãy chọn biểu mẫu nhiệm vụ trước.");
        return;
      }
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedTemplateId} onValueChange={selectTemplate}>
                ...
                <SelectContent>
                  {templates.map((template) => (
                    <SelectItem
                      key={entityId(template)}
                      value={entityId(template)}
                    >
              ...
              {selectedTemplate ? (
                <p className="text-xs text-muted-foreground">
                  ...
                  {
                    selectedTemplate.columns.filter((item) => item.visible)
                      .length
                  }{" "}
                  cột hiển thị
...
function TaskDialog({
  ...
  const submit = async () => {
    ...
    if (
      contentColumn &&
      isTemplateColumnRequired(contentColumn) &&
      !form.contentId
    ) {
    ...
    if (
      assigneeColumn &&
      isTemplateColumnRequired(assigneeColumn) &&
      !form.assigneeId
    ) {
    ...
    for (const column of dialogColumns) {
      ...
      numericFieldValues[column.key] = isNumericTemplateColumn(column, template)
        ? Number(raw)
        : raw;
      if (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>
            {edit ? "Cập nhật nhiệm vụ" : "Giao nhiệm vụ"}
          </DialogTitle>
        </DialogHeader>
            ...
            {!template ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {dialogColumns.map((column) => {
                  ...
                  const editable = canEditDialogColumn(column, userRoleCodes);
