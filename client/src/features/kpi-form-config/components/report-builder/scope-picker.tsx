...
import { Building2, ChevronDown, ChevronRight, Layers, Search } from "lucide-react";
...
import type { ScopeDraft } from "@/features/kpi-form-config/components/report-builder/report-scope";
import {
...
export function ScopePicker({
  ...
  const { data: levels = [] } = useSWR(
    enabled && value.scopeType === "by_level" ? "department-levels-scope" : null,
    fetchDepartmentLevels,
        ...
        >
          {hasChildren ? (
            <button
              ...
              aria-label={isOpen ? `Thu gọn ${node.name}` : `Mở rộng ${node.name}`}
  ...
  return (
    <div className="min-w-0 space-y-4">
          ...
          return (
            ...
            >
              <span className="flex min-w-0 items-center gap-1.5 text-sm font-medium">
                ...
                <span className="truncate">{REPORT_SCOPE_TYPE_LABEL[type]}</span>
              </span>
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block truncate font-mono text-xs text-muted-foreground">
                        {level.code}
                        {level.isKpiUnit ? " · đơn vị nhận KPI" : ""}
                      </span>
                    </span>
                  </label>
                );
      ...
      <p className="rounded-lg bg-muted/60 px-3 py-2 text-xs text-muted-foreground">
        Một đơn vị khớp nhiều mẫu thì lấy mẫu hẹp nhất: đơn vị chỉ định &gt; theo
        cấp &gt; toàn hệ thống. Nhờ vậy một năm vẫn có mẫu chung cho cả ngành và
        vài mẫu riêng cho đơn vị đặc thù.
      </p>
    </div>
  );
}
