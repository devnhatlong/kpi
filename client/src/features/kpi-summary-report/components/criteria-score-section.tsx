  ...
  */
  ...
  if (stamp && loadedId !== stamp) {
    ...
    const rowsFor = (subject: Subject) => {
      ...
      if (!next.has(key)) next.set(key, blankRows.map((row) => ({ ...row })));
      return next.get(key)!;
    };
    const apply = (
      rows: CriteriaRow[],
  ...
  const patch = (subject: Subject, criterionId: string, part: CriteriaRowPatch) =>
    setTables((prev) => {
      const key = subjectKey(subject);
    ...
    return (
      <Card>
        <CardContent className="p-4 text-sm text-muted-foreground">
          {blankRows.length
            ? "Khối A chưa được gán mẫu bảng nên chưa biết bày cột nào - cấu hình ở mục Mẫu báo cáo KPI."
            : "Danh mục tiêu chí chung chưa có dòng nào - khai ở mục Mẫu báo cáo KPI trước khi chấm khối A."}
        </CardContent>
      </Card>
    );
  }
