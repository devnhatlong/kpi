...
function emptyIndicator(index: number): KpiIndicator {
  return {
    code: `KPI-${String(index).padStart(2, "0")}`,
    name: "",
    weight: 0,
    description: "",
    criteria: "",
...
export function MasterFormAdminView() {
  ...
  const [form, setForm] = useState({
    ...
    indicators: [emptyIndicator(1), emptyIndicator(2)] as KpiIndicator[],
  });

  const weightSum = form.indicators.reduce((s, i) => s + Number(i.weight || 0), 0);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      await mutate();
      setOpen(false);
      toast.success("Đã tạo mẫu KPI (Nháp).");
    } catch (error) {
  ...
  async function handleReady(item: KpiMasterForm) {
    try {
  ...
  async function handlePublish(item: KpiMasterForm) {
    try {
  ...
  async function handleStatus(item: KpiMasterForm, status: MasterFormStatus) {
    try {
  ...
  async function handleTracking(item: KpiMasterForm) {
    try {
  ...
  async function handleDelete(item: KpiMasterForm) {
    try {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">
            Phát hành KPI cấp tỉnh
          </h1>
          <p className="text-sm text-muted-foreground">
            Tạo mẫu chỉ tiêu → chọn phạm vi phòng → phát hành (tự tạo Form KPI
            từng phòng, không qua Form 3).
          </p>
        </div>
        <Button
          onClick={() => {
            setForm({
              name: "",
              code: `KPI_${Date.now().toString(36).toUpperCase()}`,
              description: "",
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Tạo mẫu KPI cấp tỉnh</DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="KPI công tác chuyển đổi số năm 2026"
                />
              </div>
              <div className="space-y-2">
                <Label>Mã form</Label>
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setForm((s) => ({
                          ...s,
                          indicators: s.indicators.filter((_, i) => i !== index),
                        }))
                      }
                    >
                      Xoá
          ...
          {tracking ? (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">
                ...
                KPI · {tracking.phongAssigned} phòng đã phân công
              </p>
              <Table>
                <TableHeader>
                  <TableRow>
