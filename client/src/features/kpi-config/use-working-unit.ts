...
import { fetchMyWorkingDepartments, kpiConfigKeys } from "./api";

const WORKING_UNIT_KEY = "kpi-working-department-id";

export function useWorkingUnit() {
  const { data: scopedOptions = [] } = useSWR(
    [...kpiConfigKeys.sheets, "my-departments"],
    fetchMyWorkingDepartments,
    { revalidateOnFocus: false },
  );

  const [workingDepartmentId, setWorkingDepartmentIdState] = useState<string>("");
