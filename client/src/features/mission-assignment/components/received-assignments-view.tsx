...
import { AssignmentDetailDrawer } from "@/features/mission-assignment/components/assignment-detail-drawer";
import { AssignmentReportDialog } from "@/features/mission-assignment/components/assignment-report-dialog";
import { DelegateDialog } from "@/features/mission-assignment/components/delegate-dialog";
import { assignmentStatusBadgeClass } from "@/features/mission-assignment/status-styles";
import {
...
export function ReceivedAssignmentsView() {
  ...
  const [detail, setDetail] = useState<MissionAssignment | null>(null);
  const [delegating, setDelegating] = useState<MissionAssignment | null>(null);
  const [reporting, setReporting] = useState<MissionAssignment | null>(null);
  const [acting, setActing] = useState(false);

  const start = async (item: MissionAssignment) => {
    setActing(true);
    try {
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  items.map((item, index) => {
                    ...
                    const canDelegate =
                      open && item.holderType === "DEPARTMENT";
                    return (
