...
export type AssignmentTargetDepartment = {
  ...
  hasChildren: boolean;
  levelName: string;
  /** Thuộc cấp nhận nhiệm vụ - cấp gom nhóm thì chỉ để xem. */
  isUnit: boolean;
...
export type AssignmentScope = {
  ...
  /** Vai trò đã được cấu hình quyền giao nhiệm vụ chưa. */
  isEnabled: boolean;
  requireApproval: boolean;
  scopes: string[];
};
...
export function holderLabel(item: MissionAssignment): string {
