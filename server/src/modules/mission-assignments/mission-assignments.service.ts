...
import {
  MissionScopeConfigService,
  type EffectiveScope,
} from '@/modules/mission-scope-config/mission-scope-config.service';
import {
  ApproveMissionAssignmentDto,
  CreateMissionAssignmentBatchDto,
  CreateMissionAssignmentDto,
...
export class MissionAssignmentsService {
  constructor(
    @InjectModel(MissionAssignment.name)
    private readonly assignmentModel: Model<MissionAssignmentDocument>,
    ...
    private readonly scopeConfigService: MissionScopeConfigService,
  ) {}
  ...
  async create(
    userId: string,
    roleCodes: string[],
    dto: CreateMissionAssignmentBatchDto,
  ) {
    const actor = await this.resolveActor(userId, roleCodes);
    if (!actor.scope.isEnabled) {
      throw new ForbiddenException(
        'Vai trò của bạn chưa được cấu hình quyền giao nhiệm vụ.',
      );
    }
  ...
  async delegate(
    ...
    roleCodes: string[],
    id: string,
    dto: DelegateMissionAssignmentDto,
  ) {
    const actor = await this.resolveActor(userId, roleCodes);
  ...
  async report(
    ...
    roleCodes: string[],
    id: string,
    dto: ReportMissionAssignmentDto,
  ) {
    const actor = await this.resolveActor(userId, roleCodes);
    const item = await this.requireHeldByActor(actor, id);
  ...
  async approve(
    ...
    roleCodes: string[],
    id: string,
    dto: ApproveMissionAssignmentDto,
  ) {
    const actor = await this.resolveActor(userId, roleCodes);
  ...
  async reject(
    ...
    roleCodes: string[],
    id: string,
    dto: RejectMissionAssignmentDto,
  ) {
    const actor = await this.resolveActor(userId, roleCodes);
    const item = await this.requireAssignedByActor(actor, id);
    if (item.status !== 'SUBMITTED') {
  ...
  async findReceived(
    ...
    const actor = await this.resolveActor(userId, roleCodes);
    const holderOr: Record<string, unknown>[] = [
  ...
  async findTargets(userId: string, roleCodes: string[]) {
    ...
    const unitLevelIds = await this.getMissionUnitLevelIds();
  ...
  private canSee(actor: Actor, item: MissionAssignmentDocument) {
    const ids = [
      item.holderUserId,
      item.issuerId,
  ...
  private async requireCatalog(
    ...
    if (!scoreGroup || !scoreGroup.isActive) {
      throw new BadRequestException(
        `${at}nhóm điểm không tồn tại hoặc đã ngừng.`,
      );
    }
  }
  ...
  private async assertValidDepartmentTarget(
    ...
    const direct = this.isEffectiveDirectChild(
      target,
      fromDepartmentId,
      unitIds,
    );
  ...
  /**
   * Các cấp đơn vị nhận nhiệm vụ (Phòng, Xã...). Cấp không bật cờ chỉ để gom nhóm.
   * Chưa cấp nào được bật thì coi như mọi cấp đều nhận được, để hệ thống vẫn
   * dùng được trước khi super admin cấu hình.
   */
  private async getMissionUnitLevelIds(): Promise<Set<string> | null> {
    const levels = await this.departmentLevelModel
      .find({ isMissionUnit: true, isActive: true })
      .select('_id');
