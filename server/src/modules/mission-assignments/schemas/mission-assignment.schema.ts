...
import { ScoreGroup } from '@/modules/mission-form-config/schemas/score-group.schema';
import { User } from '@/modules/users/schemas/user.schema';

export type MissionAssignmentDocument = HydratedDocument<MissionAssignment>;
 ...
 */
@Schema({ timestamps: true, collection: 'mission_assignments' })
...
export const MissionAssignmentSchema =
  SchemaFactory.createForClass(MissionAssignment);

MissionAssignmentSchema.index({
  holderDepartmentId: 1,
  status: 1,
  createdAt: -1,
});
MissionAssignmentSchema.index({ holderUserId: 1, status: 1, createdAt: -1 });
MissionAssignmentSchema.index({ issuerId: 1, createdAt: -1 });
MissionAssignmentSchema.index({ batchId: 1, createdAt: -1 });
