...
export class AssignmentTargetsDto {
  @ApiPropertyOptional({
    type: [String],
    description: 'Đơn vị nhận - nhiệm vụ nằm ở đơn vị, do quản trị đơn vị xử lý',
  })
  @IsOptional()
  @IsArray()
...
export class CreateKpiAssignmentBatchDto {
  @ApiProperty({ type: [CreateKpiAssignmentDto] })
  @IsArray({ message: 'items phải là mảng.' })
  @ArrayNotEmpty({ message: 'Chưa có nhiệm vụ nào để giao.' })
  @ValidateNested({ each: true })
  @Type(() => CreateKpiAssignmentDto)
  items!: CreateKpiAssignmentDto[];
}

export class DelegateKpiAssignmentDto {
...
export class ReportKpiAssignmentDto {
...
export class ApproveKpiAssignmentDto {
...
export class RejectKpiAssignmentDto {
