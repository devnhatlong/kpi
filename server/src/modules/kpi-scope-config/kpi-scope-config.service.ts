...
import {
  ...
  defaultSeedFor,
  type KpiScope,
} from './kpi-scope.constants';
import {
  KpiScopeConfig,
  KpiScopeConfigDocument,
...
@Injectable()
export class KpiScopeConfigService implements OnModuleInit {
  constructor(
    @InjectModel(KpiScopeConfig.name)
    private readonly configModel: Model<KpiScopeConfigDocument>,
  ...
  private async seedDefaults() {
    for (const seed of KPI_SCOPE_SEEDS) {
  ...
  async findAll() {
    ...
    return { data: { items, scopeMeta: KPI_SCOPE_META } };
  }

  async save(dto: SaveKpiScopeConfigDto) {
    ...
    return { message: 'Đã lưu cấu hình phạm vi giao KPI.' };
  }
  ...
  async resetToDefault() {
    ...
    await this.configModel.deleteMany({
      roleCode: { $nin: KPI_SCOPE_SEEDS.map((item) => item.roleCode) },
    });
    return this.findAll();
  }
