...
export const MISSION_SCOPE_META: MissionScopeMeta[] = [
  {
    key: 'SELF',
    group: 'PERSON',
    label: 'Chính mình',
...
export const MISSION_SCOPE_SEEDS: MissionScopeSeed[] = [
  {
    // Chỉ để cấu hình hệ thống, không tham gia giao/nhận nhiệm vụ nghiệp vụ.
    roleCode: RoleCode.SUPER_ADMIN,
    isEnabled: false,
    scopes: [],
    requireApproval: true,
    note: 'Tài khoản cấu hình hệ thống - không giao nhiệm vụ. Việc đó của Công an tỉnh.',
  },
  {
...
export function defaultSeedFor(roleCode: string): MissionScopeSeed {
  return (
    MISSION_SCOPE_SEEDS.find((item) => item.roleCode === roleCode) ?? {
      roleCode,
      isEnabled: false,
      scopes: [],
      requireApproval: true,
