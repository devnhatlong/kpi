...
import {
  PERSONAL_MISSION_REVIEW_STATUSES,
  PersonalMissionProgressLog,
  PersonalMissionProgressLogSchema,
  type PersonalMissionReviewStatus,
} from './personal-mission-item.schema';

export type PersonalMissionCriteriaSheetDocument =
  HydratedDocument<PersonalMissionCriteriaSheet>;
...
export class PersonalMissionCriterionRow {
...
export const PersonalMissionCriterionRowSchema = SchemaFactory.createForClass(
 ...
 * bộ đó - xem `MissionSummaryReport.criteriaScores`.
 */
@Schema({ timestamps: true, collection: 'personal_mission_criteria_sheets' })
export class PersonalMissionCriteriaSheet {
  ...
  @Prop({ type: [PersonalMissionCriterionRowSchema], default: [] })
  rows!: PersonalMissionCriterionRow[];
...
export const PersonalMissionCriteriaSheetSchema = SchemaFactory.createForClass(
  PersonalMissionCriteriaSheet,
);

// Mỗi cán bộ mỗi THÁNG đúng một bảng - hai bảng thì không biết cộng bản nào.
PersonalMissionCriteriaSheetSchema.index(
