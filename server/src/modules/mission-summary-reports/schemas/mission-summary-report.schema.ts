...
import { PersonalMissionItem } from '@/modules/personal-mission/schemas/personal-mission-item.schema';
import { User } from '@/modules/users/schemas/user.schema';

export type MissionSummaryReportDocument =
  HydratedDocument<MissionSummaryReport>;

/**
 * Đời của một báo cáo tổng hợp, chạy đúng chuỗi duyệt của nhiệm vụ cá nhân:
 ...
 */
export const MISSION_SUMMARY_REPORT_STATUSES = [
  'DRAFT',
  'SENT',
  'RETURNED',
  'APPROVED',
...
export type MissionSummaryReportStatus =
  (typeof MISSION_SUMMARY_REPORT_STATUSES)[number];
 ...
 */
export const MISSION_SUMMARY_LOG_TYPES = [
  'CREATE',
  'UPDATE',
  'ADD_ITEMS',
  'REMOVE_ITEMS',
...
export type MissionSummaryLogType = (typeof MISSION_SUMMARY_LOG_TYPES)[number];

/**
 * Nhiệm vụ tự nhập: việc có thật nhưng không đi qua nhiệm vụ cá nhân (việc đột xuất,
 * việc của đơn vị phối hợp). Chép thẳng nội dung vào báo cáo vì không có bản
 * ghi gốc nào để trỏ tới.
 */
@Schema({ _id: true, timestamps: false })
export class MissionSummaryManualItem {
...
export const MissionSummaryManualItemSchema = SchemaFactory.createForClass(
 ...
 *   vào điểm của đơn vị.
 */
export const MISSION_CRITERION_SUBJECT_TYPES = ['DEPARTMENT', 'USER'] as const;
export type MissionCriterionSubjectType =
  (typeof MISSION_CRITERION_SUBJECT_TYPES)[number];
...
export class MissionSummaryCriterionScore {
  @Prop({
    type: String,
    enum: MISSION_CRITERION_SUBJECT_TYPES,
    required: true,
  })
...
export const MissionSummaryCriterionScoreSchema = SchemaFactory.createForClass(
  MissionSummaryCriterionScore,
);

@Schema({ _id: false, timestamps: false })
export class MissionSummaryReportLog {
  @Prop({ type: String, enum: MISSION_SUMMARY_LOG_TYPES, required: true })
...
export const MissionSummaryReportLogSchema = SchemaFactory.createForClass(
  MissionSummaryReportLog,
);

/**
 * Báo cáo tổng hợp: một tập nhiệm vụ cá nhân ĐÃ HOÀN THÀNH được người lập
 ...
 */
@Schema({ timestamps: true, collection: 'mission_summary_reports' })
export class MissionSummaryReport {
  @Prop({ required: true, trim: true, maxlength: 300 })
  title!: string;
  ...
  @Prop({
    type: [{ type: Types.ObjectId, ref: PersonalMissionItem.name }],
    default: [],
    index: true,
  })
  ...
  /**
   * Số nhiệm vụ lấy từ nhiệm vụ cá nhân, chép lại để danh sách báo cáo khỏi phải nạp cả mảng
   * itemIds. Luôn ghi cùng lúc với itemIds.
   */
  @Prop({ default: 0, min: 0 })
  itemCount!: number;

  @Prop({ type: [MissionSummaryManualItemSchema], default: [] })
  manualItems!: MissionSummaryManualItem[];
   ...
   * người lập chưa chấm.
   */
  @Prop({ type: [MissionSummaryCriterionScoreSchema], default: [] })
  criteriaScores!: MissionSummaryCriterionScore[];

  @Prop({ type: [MissionSummaryReportLogSchema], default: [] })
  logs!: MissionSummaryReportLog[];

  @Prop({
    type: String,
    enum: MISSION_SUMMARY_REPORT_STATUSES,
    default: 'DRAFT',
    index: true,
  })
  status!: MissionSummaryReportStatus;
...
export const MissionSummaryReportSchema =
  SchemaFactory.createForClass(MissionSummaryReport);

/** Danh sách báo cáo của tôi, mới nhất trước. */
MissionSummaryReportSchema.index({ ownerId: 1, createdAt: -1 });

/** Hộp thư đến của cấp trên: báo cáo cấp dưới trình lên mình. */
MissionSummaryReportSchema.index({ sentToId: 1, createdAt: -1 });

/** Tra ngược "nhiệm vụ này đã nằm trong báo cáo nào chưa". */
MissionSummaryReportSchema.index({ itemIds: 1, ownerId: 1 });
